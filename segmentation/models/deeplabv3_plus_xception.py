...
from segmentation.utils.helpers import initialize_weights,set_trainable
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, rate=1):
        ...
        if self._without_padding:
            ...
            self._conv = torch.nn.Conv2d(in_channels,
                                         ...
                                         dilation=rate,
                                         bias=False)
        self._batch_norm = torch.nn.InstanceNorm2d(out_channels, 
                                                **_BATCH_NORM_PARAMS)
    ...
    def __init__(self, in_channels, out_channels, kernel_size, 
        ...
        if self._without_padding:
            ...
            self._conv_depthwise = torch.nn.Conv2d(in_channels,
                                                   ...
                                                   bias=False,
                                                   **kwargs)
        self._batch_norm_depthwise = torch.nn.InstanceNorm2d(
        ...
        self._conv_pointwise = torch.nn.Conv2d(out_channels_depthwise,
                                               ...
                                               bias=False,
                                               **kwargs)
        self._batch_norm_pointwise = torch.nn.InstanceNorm2d(
    ...
    def __init__(self, in_channels, depth_list, skip_connection_type, stride, 
        ...
        if skip_connection_type == 'conv':
            self._conv_skip_connection = torch.nn.Conv2d(in_channels,
                                                         ...
                                                         kernel_size=1,
                                                         stride=stride)
            self._batch_norm_shortcut = torch.nn.InstanceNorm2d(
...
def xception_65(num_classes=None,
    ...
    xception = Xception65(num_classes=num_classes, global_pool=global_pool, 
                          ...
                          scope=scope)
    if pretrained:
        _load_state_dict(xception, num_classes, checkpoint_path=checkpoint_path)
    return xception
...
class ASSP(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
        print("HERE ASSP", x.shape, x1.shape, x2.shape, x3.shape, x4.shape)
        x5 = F.interpolate(self.avg_pool(x), size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)

        print("HERE ASSP", x.shape, x1.shape, x2.shape, x3.shape, x4.shape, x5.shape)
        x = self.conv1(torch.cat((x1, x2, x3, x4, x5), dim=1))
        print("HERE ASSP2", x.shape)
        #x = self.bn1(x)
        print("HERE ASSP3", x.shape)
        x = self.dropout(self.relu(x))
        print("HERE ASSP4", x.shape)
...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_classes):
        ...
        self.output = nn.Sequential(
            nn.Conv2d(48+256, 256*2, 3, stride=1, padding=1, bias=False),
            #nn.InstanceNorm2d(256*2),
            nn.ReLU(inplace=True),
            nn.Conv2d(256*2, 256*3, 3, stride=1, padding=1, bias=False),
            #nn.InstanceNorm2d(256*3),
            nn.SELU(inplace=True),
            nn.Dropout(0.1),
            nn.Conv2d(256*3, num_classes, 1, stride=1),
        )
        initialize_weights(self)
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=True, 
                output_stride=16, freeze_bn=False,freeze_backbone=False, checkpoint_path=",", **_):
        ...
        if 'resnet' in backbone:
            ...
            print(os.path.join(checkpoint_path, 'xception_65.pth'))
            self.backbone = xception_65(output_stride=output_stride, pretrained=pretrained,global_pool=False,checkpoint_path=os.path.join(checkpoint_path, 'xception_65.pth'))
    ...
    def freeze_bn(self):
        for module in self.modules():
            if isinstance(module, nn.InstanceNorm2d): module.eval()
